...
struct UniformPriorWeightedSampling{T<:Real,I<:Integer,Q<:AbstractVecOrMat{<:Real}} <:
    ...
    function UniformPriorWeightedSampling{T,I,Q}(
        ...
        [bin_occupancy[i] = sum(binnums .== i) for i = 1:n_bins]
...
function generate_stimulus(s::Bernoulli)
    ...
    [spect[binnum.==i] .= binned_repr[i] for i = 1:(s.n_bins)]
...
function generate_stimulus(s::UniformPriorWeightedSampling)
    ...
    filled_bins =
        sample(1:(s.n_bins), Weights(s.bin_probs), n_bins_to_fill; replace = false)
