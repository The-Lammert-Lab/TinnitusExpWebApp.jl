...
function unique_for_usr_and_exp(field::Symbol, m::T)::ValidationResult where {T<:AbstractModel}
    obj = findone(typeof(m); 
    ...
    if (obj !== nothing && !ispersisted(m))
        return ValidationResult(invalid, :unique_for_usr_and_exp, "already exists for this experiment")
    end
